...
func RegisterScriptRoutes(r *gin.Engine) {
    ...
    fetch(url)
        .then(res => res.json())
        .then(data => {
            const fetchedVisitors = parseInt(data.visitors || '0', 10); 
            const fetchedViews = parseInt(data.pageviews || '0', 10); 

            const visitors = fetchedVisitors + initVisitors;
            const views = fetchedViews + initViews;
